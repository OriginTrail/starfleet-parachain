...
//! ## Overview
//!
//! Evm Accounts pallet provide a two way mapping between Substrate accounts and
...
use pallet_evm::{AddressMapping as EvmPalletAddressMappingTrait};
...
pub trait AddressMapping<AccountId32> {
	/// Returns the AccountId used go generate the given EvmAddress.
	fn get_account_id(evm: &EvmAddress) -> AccountId32;
...
impl<T: Config> Pallet<T> {
	...
	fn evm_account_domain_separator() -> [u8; 32] {
		...
		domain_seperator_msg.extend_from_slice(keccak256!("OTP EVM claim")); // name
...
impl<T: Config> EvmPalletAddressMappingTrait<T::AccountId> for EvmAddressMapping<T>
where
